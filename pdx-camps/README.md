# Portland Campsite Enforcement Data

Accessed directly from Portland Police Bureau's Open Data initiative.

**Source:** [Public Camping Ordinance Enforcement](https://public.tableau.com/app/profile/portlandpolicebureau/viz/PublicCampingOrdinanceEnforcement/PPBCampsiteCleanups)

---

## About This Dataset

One flat row per **neighborhood per month**. Each month's network sweep pins the dashboard's date filter to a single month and walks every Portland neighborhood, capturing campsite-cleanup activity, contact outcomes, and enforcement actions. Neighborhoods with no reported activity in a given month are omitted entirely (not stored as zeros). Within an active neighborhood-month, a category that reported nothing is recorded as `0`, so every count cell is a concrete integer.

The closed-domain breakdowns from the source dashboard (dwelling type, contact outcome, charge type, warrant type) are pivoted into named columns. Note that the enforcement and warrant measures are **parallel statistics**, not sub-totals of the cleanup count, they will not reconcile against `cleanup_incidents`.
